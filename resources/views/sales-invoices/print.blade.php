...
<main class="sheet" style="{{ $otherFontCss }}">
    ...
    @empty <tr><td colspan="{{ 5 + (int) $showDiscount + (int) $showTax }}">No invoice items</td></tr> @endforelse
    </tbody></table>
    <section class="bottom">
        ...
        <div class="summary"><div class="summary-row"><span>Subtotal</span><span>{{ app_money($subtotal) }}</span></div>@if($showDiscount)<div class="summary-row"><span>Discount</span><span>{{ app_money($discount) }}</span></div>@endif @if($showTax)<div class="summary-row divider"><span>Tax</span><span>{{ app_money((float) $invoiceTotals['vat_total']) }}</span></div>@endif<div class="summary-row grand"><span>Grand Total</span><span>{{ app_money((float) $invoiceTotals['total']) }}</span></div><div class="summary-row divider"><span>Paid</span><span>{{ app_money((float) $paidAmount) }}</span></div><div class="summary-row due"><span>Amount Due</span><span>{{ app_money((float) $dueAmount) }}</span></div></div>
    </section>
    @if($showNote)<section class="notes"><h2 class="section-title">Notes</h2>@if(filled($invoice->notes))<div>{{ \Filament\Forms\Components\RichEditor\RichContentRenderer::make($invoice->notes) }}</div>@endif @if($receiptNote !== '')<div>{{ \Filament\Forms\Components\RichEditor\RichContentRenderer::make($receiptNote) }}</div>@endif</section>@endif
