...
<head>
    ...
    <title>Purchase Invoice - {{ $invoice->voucherNumber() }}</title>
    <style>
...
@php
    ...
    $supplierAddress = $supplier?->address ?: collect([$supplier?->address_line1, $supplier?->address_line2, $supplier?->city, $supplier?->postcode, $supplier?->country])->filter()->join(', ');
    $supplierVat = $supplier?->tax_number ?: $supplier?->vat_number;
    $supplierPhone = $supplier?->phone ?: ($supplier?->telephone_no ?: $supplier?->mobile_no);
    $subtotal = (float) $invoiceTotals['subtotal'];
    $discount = (float) $invoiceTotals['discount'];
    $money = fn (float|int|string|null $amount): string => \App\Support\CurrencyFormatter::formatForCurrency($amount, $invoice->currency_id ?: $supplier?->currency_id);
...
@endphp
<header class="toolbar"><div class="toolbar-title">Purchase Invoice</div><div class="toolbar-actions"><a href="{{ url()->previous() }}">Back</a><button type="button" onclick="window.print()">Print Invoice</button></div></header>
<main class="sheet">
    ...
    <table class="info-table"><thead><tr><th>Voucher No</th><th>Supplier Invoice No</th><th>Invoice Date</th><th>Due Date</th></tr></thead><tbody><tr><td>{{ $invoice->voucherNumber() }}</td><td>{{ $invoice->supplierInvoiceNumber() ?: '—' }}</td><td>{{ $invoice->invoice_date?->format('d M Y') ?: '—' }}</td><td>{{ $invoice->due_date?->format('d M Y') ?: '—' }}</td></tr></tbody></table>
    <section class="addresses">
        <div class="address-card"><h2 class="section-title">Supplier</h2><div class="address-name">{{ $supplier?->company_name ?: ($supplier?->name ?: 'Supplier') }}</div>@if($supplierAddress)<div class="address-detail">{{ $supplierAddress }}</div>@endif @if($supplier?->email)<div class="address-detail">{{ $supplier->email }}</div>@endif @if($supplierVat)<div class="address-detail"><span class="detail-label">VAT No:</span> {{ $supplierVat }}</div>@endif</div>
        <div class="address-card"><h2 class="section-title">Supplier Contact</h2><div class="address-name">{{ $supplier?->contact_person ?: ($supplier?->company_name ?: ($supplier?->name ?: 'Supplier')) }}</div>@if($supplierPhone)<div class="address-detail">{{ $supplierPhone }}</div>@endif @if($supplier?->email)<div class="address-detail">{{ $supplier->email }}</div>@endif @if($supplier?->website)<div class="address-detail">{{ $supplier->website }}</div>@endif</div>
    </section>
    ...
    @endif
    <footer class="footer">This is a system-generated purchase invoice and no signature is required.</footer>
</main>
</body>
</html>
