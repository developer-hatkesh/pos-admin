...
<main class="sheet" style="{{ $otherFontCss }}">
    ...
    @if($showNote)<section class="notes"><h2 class="section-title">Notes</h2>@if(filled($estimate->notes))<div>{{ $estimate->notes }}</div>@endif @if($receiptNote !== '')<div>{{ $receiptNote }}</div>@endif</section>@endif
